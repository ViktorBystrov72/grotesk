...
async def get_job_detail(
    ...
    return JobDetailResponse(
        ...
        history=[
            JobHistoryRecordResponse(status=record.status.value, message=record.message) for record in job.history
        ],
...
async def download_job_artifact(
    ...
    if artifact_path.suffix.lower() == ".json" and job.result_type == "transcription":
